# Index Of fantasyui-com Repositories

## Table of Contents
- [Advanced Research](#advanced-research)
- [Algorithm](#algorithm)
- [Analysis](#analysis)
- [App Store](#app-store)
- [App Store Catalog](#app-store-catalog)
- [Application](#application)
- [Art](#art)
- [Audio Samples](#audio-samples)
- [Awesome List](#awesome-list)
- [Boilerplate](#boilerplate)
- [Bootstrap](#bootstrap)
- [Bootstrap Analysis](#bootstrap-analysis)
- [Business](#business)
- [CLI](#cli)
- [CSS](#css)
- [Catalog](#catalog)
- [Code Generator](#code-generator)
- [Crypto](#crypto)
- [DRM](#drm)
- [Dashboard](#dashboard)
- [Data Faker](#data-faker)
- [Diff](#diff)
- [Electron](#electron)
- [Event Emitter](#event-emitter)
- [File Management](#file-management)
- [Files](#files)
- [Fitness](#fitness)
- [Hardened](#hardened)
- [Index Of](#index-of)
- [JSON](#json)
- [Language](#language)
- [Launcher](#launcher)
- [Library](#library)
- [MUD](#mud)
- [Multiprocess](#multiprocess)
- [Music](#music)
- [Network](#network)
- [Node](#node)
- [Object Database](#object-database)
- [Ping](#ping)
- [Poem](#poem)
- [Printing](#printing)
- [Proxy](#proxy)
- [Resume](#resume)
- [Robots](#robots)
- [Server](#server)
- [Shell](#shell)
- [Show and Tell](#show-and-tell)
- [Smart CSS](#smart-css)
- [Sort](#sort)
- [StackScript](#stack-script)
- [Standard Library](#standard-library)
- [UI](#ui)
- [URL](#url)
- [VFS](#vfs)
- [Video](#video)
- [Web Application](#web-application)
- [Web Design](#web-design)
- [WikiWiki](#wiki-wiki)
- [Index](#index)

## Advanced Research
- [cgui](https://github.com/fantasyui-com/cgui) - Conversational Graphic User Interface. 
- [recombinator](https://github.com/fantasyui-com/recombinator) - CSS recombination system. 

## Algorithm
- [aquarium](https://github.com/fantasyui-com/aquarium) - A simple proportional value distribution library aimed at performing probability driven experiments in an enclosed virtual space by means of virtual objects and user defined uncertainty. 
- [decrement](https://github.com/fantasyui-com/decrement) - Auto decrement value at preset rate. 
- [diff-css](https://github.com/fantasyui-com/diff-css) - Command to compare two CSS files and list changes. 
- [ls-html](https://github.com/fantasyui-com/ls-html) - List information about HTML file. 
- [oneof](https://github.com/fantasyui-com/oneof) - Return a random item from a list. 

## Analysis
- [bootstrap-monitor](https://github.com/fantasyui-com/bootstrap-monitor) - Bootstrap 4 monitoring system designed to help tracking class changes. 
- [list-css-classes](https://github.com/fantasyui-com/list-css-classes) - Node npm module used to parse a CSS string and list the CSS classes defined inside. 
- [ls-html](https://github.com/fantasyui-com/ls-html) - List information about HTML file. 

## App Store
- [app-store](https://github.com/fantasyui-com/app-store) - Flexible general purpose app-store framework with customizable app feeds. 
- [application-launcher](https://github.com/fantasyui-com/application-launcher) - Easily install and launch electron applications from npm. Just set your electron application npm-name in configuration.json and ship the zips (Linux, Mac, Win). Launcher (which is electron+npm) will keep installing the latest version of your module each time it is executed. 
- [business-software](https://github.com/fantasyui-com/business-software) - Open source app store for Business Managers. 
- [physical-fitness](https://github.com/fantasyui-com/physical-fitness) - Open source app store for nerds and bodybuilders. 
- [readme-editor](https://github.com/fantasyui-com/readme-editor) - Super simple readme editor (app-store example package). 
- [writing-workshop](https://github.com/fantasyui-com/writing-workshop) - Open source app store for poets, writers and philosophers. 

## App Store Catalog
- [app-catalog](https://github.com/fantasyui-com/app-catalog) - Application catalog for npm's app-store. 

## Application
- [app-store](https://github.com/fantasyui-com/app-store) - Flexible general purpose app-store framework with customizable app feeds. 
- [application-launcher](https://github.com/fantasyui-com/application-launcher) - Easily install and launch electron applications from npm. Just set your electron application npm-name in configuration.json and ship the zips (Linux, Mac, Win). Launcher (which is electron+npm) will keep installing the latest version of your module each time it is executed. 
- [bootstrap-dashboard](https://github.com/fantasyui-com/bootstrap-dashboard) - Fully Functional Pure Bootstrap Dashboard, powered by EventEmitter, npm, Node and Electron. 
- [botnet](https://github.com/fantasyui-com/botnet) - Electron based communication program driven by robotic network entities inspired by email. 
- [business-software](https://github.com/fantasyui-com/business-software) - Open source app store for Business Managers. 
- [color-schemes](https://github.com/fantasyui-com/color-schemes) - Color Scheme Explorer (GUI Application). 
- [commode](https://github.com/fantasyui-com/commode) - Electron-Vue based, iTunes like UI, for file tagging, restructuring, processing, and general management. 
- [cornwall](https://github.com/fantasyui-com/cornwall) - Electron-based PixiJS Video Recording Environment. 
- [cylinder](https://github.com/fantasyui-com/cylinder) - Electron based digital audio workstation piano roll. 
- [giblets](https://github.com/fantasyui-com/giblets) - Musical instrument based on giblet sound emitter. 
- [harlequin](https://github.com/fantasyui-com/harlequin) - Live Music Programming Environment. 
- [illustrator](https://github.com/fantasyui-com/illustrator) - Command line tool to extract drawing reference from a photo and create a pdf formatted drawing reference booklet. A tool for artists of all ages. 
- [mockup](https://github.com/fantasyui-com/mockup) - Perspective app screens and isometric mock-up tool. Automated high quality, high resolution, marketing art designer for mockups. Written in minimalist Bash using ImageMagick and Love! 
- [osmose](https://github.com/fantasyui-com/osmose) - Electron application for easy color balancing and filtering. 
- [pacom](https://github.com/fantasyui-com/pacom) - Minimalist WikiWiki with an english-like programming language. 
- [page-setup](https://github.com/fantasyui-com/page-setup) - Electron application for decorating and printing plain text files. 
- [photograph](https://github.com/fantasyui-com/photograph) - Wrap web application UI in a pretty frame, prior to screen capture and posting to show and tell sites. 
- [physical-fitness](https://github.com/fantasyui-com/physical-fitness) - Open source app store for nerds and bodybuilders. 
- [readme-editor](https://github.com/fantasyui-com/readme-editor) - Super simple readme editor (app-store example package). 
- [shortwave](https://github.com/fantasyui-com/shortwave) - Electron based music beat producer, powered by Node Emitter, Howler, Vue and Bootstrap. 
- [writing-workshop](https://github.com/fantasyui-com/writing-workshop) - Open source app store for poets, writers and philosophers. 
- [yip](https://github.com/fantasyui-com/yip) - Electron backed music player based on the howler audio library. 

## Art
- [illustrator](https://github.com/fantasyui-com/illustrator) - Command line tool to extract drawing reference from a photo and create a pdf formatted drawing reference booklet. A tool for artists of all ages. 
- [mockup](https://github.com/fantasyui-com/mockup) - Perspective app screens and isometric mock-up tool. Automated high quality, high resolution, marketing art designer for mockups. Written in minimalist Bash using ImageMagick and Love! 

## Audio Samples
- [ultrasonic](https://github.com/fantasyui-com/ultrasonic) - A collection of outstanding sound samples extracted from sonic-pi project and made compatible with npm modules for use with node. 

## Awesome List
- [awesome-gui](https://github.com/fantasyui-com/awesome-gui) - Awesome list of GUI concepts. 
- [awesome-http](https://github.com/fantasyui-com/awesome-http) - Awesome HTTP. 
- [awesome-ideas](https://github.com/fantasyui-com/awesome-ideas) - Awesome programming ideas and inspiration from all around the world of languages and patterns. 
- [awesome-publishing](https://github.com/fantasyui-com/awesome-publishing) - Awesome List of Modern Publishing Resources. 
- [awesome](https://github.com/fantasyui-com/awesome) - A short list of awesome people. 

## Boilerplate
- [bootstrap-electron](https://github.com/fantasyui-com/bootstrap-electron) - Bootstrap based Electron Application. 

## Bootstrap
- [bootstrap-cards](https://github.com/fantasyui-com/bootstrap-cards) - Infinite Number of Bootstrap Cards. 
- [bootstrap-containers](https://github.com/fantasyui-com/bootstrap-containers) - Bootstrap Container Components. 
- [bootstrap-electron](https://github.com/fantasyui-com/bootstrap-electron) - Bootstrap based Electron Application. 
- [bootstrap-faker](https://github.com/fantasyui-com/bootstrap-faker) - Bootstrap Faker generates massive amounts of useful randomized bootstrap related data such as card styles, warning labels, button types, etc. 
- [bootstrap-styleguide](https://github.com/fantasyui-com/bootstrap-styleguide) - Bootstrap Styleguide generator. Create bootstrap style guides by scraping documentation. 
- [card-generator](https://github.com/fantasyui-com/card-generator) - Bootstrap Card Generator. 
- [container-generator](https://github.com/fantasyui-com/container-generator) - Bootstrap Container Generator. 

## Bootstrap Analysis
- [bootstrap-monitor](https://github.com/fantasyui-com/bootstrap-monitor) - Bootstrap 4 monitoring system designed to help tracking class changes. 
- [bootstrap-styleguide](https://github.com/fantasyui-com/bootstrap-styleguide) - Bootstrap Styleguide generator. Create bootstrap style guides by scraping documentation. 

## Business
- [catalog-server](https://github.com/fantasyui-com/catalog-server) - Commercial product management server. 
- [chub](https://github.com/fantasyui-com/chub) - Crate subscription, management and shipping business in a box. 
- [food-delivery](https://github.com/fantasyui-com/food-delivery) - Massively Distributed Food Delivery Service Framework, 
- [online-marketplace](https://github.com/fantasyui-com/online-marketplace) - Simple online marketplace for selling files. 
- [vicious](https://github.com/fantasyui-com/vicious) - Business in a box server. Inspired by google result pages driven via payments by stripe. 

## CLI
- [bootstrap-styleguide](https://github.com/fantasyui-com/bootstrap-styleguide) - Bootstrap Styleguide generator. Create bootstrap style guides by scraping documentation. 
- [changeline](https://github.com/fantasyui-com/changeline) - Search and replace in lines of files across directories. 
- [csshue](https://github.com/fantasyui-com/csshue) - CLI utility to uniformly rotate hue of colors in a css file. 
- [epx](https://github.com/fantasyui-com/epx) - Electron Package Execute (npx but for electron apps). 
- [illustrator](https://github.com/fantasyui-com/illustrator) - Command line tool to extract drawing reference from a photo and create a pdf formatted drawing reference booklet. A tool for artists of all ages. 
- [kebab-files](https://github.com/fantasyui-com/kebab-files) - Smartly normalize filenames in a directory by stripping down filenames to alphanumeric characters with hyphens. 
- [larceny](https://github.com/fantasyui-com/larceny) - Transport select remote CSS declarations into the rule of a local css file (Select and inject remote CSS code into your local CSS file). 
- [line-filter](https://github.com/fantasyui-com/line-filter) - Command that controls echoing of lines from source files based on whitelists and blacklists. 
- [list-html-classes](https://github.com/fantasyui-com/list-html-classes) - Node npm module used to parse HTML string and list the CSS classes. 
- [ls-css](https://github.com/fantasyui-com/ls-css) - List information about CSS file. 
- [recombinator](https://github.com/fantasyui-com/recombinator) - CSS recombination system. 
- [revolutionize](https://github.com/fantasyui-com/revolutionize) - PostCSS based web application skinning strategy. 
- [theft](https://github.com/fantasyui-com/theft) - Means of transporting CSS rule declarations from one file to another. 

## CSS
- [aquarium](https://github.com/fantasyui-com/aquarium) - A simple proportional value distribution library aimed at performing probability driven experiments in an enclosed virtual space by means of virtual objects and user defined uncertainty. 
- [cgui](https://github.com/fantasyui-com/cgui) - Conversational Graphic User Interface. 
- [csshue](https://github.com/fantasyui-com/csshue) - CLI utility to uniformly rotate hue of colors in a css file. 
- [decrement](https://github.com/fantasyui-com/decrement) - Auto decrement value at preset rate. 
- [diff-css](https://github.com/fantasyui-com/diff-css) - Command to compare two CSS files and list changes. 
- [larceny](https://github.com/fantasyui-com/larceny) - Transport select remote CSS declarations into the rule of a local css file (Select and inject remote CSS code into your local CSS file). 
- [list-css-classes](https://github.com/fantasyui-com/list-css-classes) - Node npm module used to parse a CSS string and list the CSS classes defined inside. 
- [list-html-classes](https://github.com/fantasyui-com/list-html-classes) - Node npm module used to parse HTML string and list the CSS classes. 
- [ls-css](https://github.com/fantasyui-com/ls-css) - List information about CSS file. 
- [osmose](https://github.com/fantasyui-com/osmose) - Electron application for easy color balancing and filtering. 
- [recombinator](https://github.com/fantasyui-com/recombinator) - CSS recombination system. 
- [revolutionize](https://github.com/fantasyui-com/revolutionize) - PostCSS based web application skinning strategy. 
- [select-declarations](https://github.com/fantasyui-com/select-declarations) - Select contents of rule declarations via CSS selectors. 
- [skins](https://github.com/fantasyui-com/skins) - Library of Industry Standard CSS Designs. 
- [spacers](https://github.com/fantasyui-com/spacers) - Lightweight CSS spacing classes inspired by bootstrap generated with precss. 
- [squirm](https://github.com/fantasyui-com/squirm) - CSS Color Processing Library. 
- [theft](https://github.com/fantasyui-com/theft) - Means of transporting CSS rule declarations from one file to another. 
- [web-fonts](https://github.com/fantasyui-com/web-fonts) - PostCSS web font injector (PostCSS plugin). 

## Catalog
- [skins](https://github.com/fantasyui-com/skins) - Library of Industry Standard CSS Designs. 

## Code Generator
- [card-generator](https://github.com/fantasyui-com/card-generator) - Bootstrap Card Generator. 
- [container-generator](https://github.com/fantasyui-com/container-generator) - Bootstrap Container Generator. 
- [spqr](https://github.com/fantasyui-com/spqr) - Webpage Design Generator. 

## Crypto
- [certification](https://github.com/fantasyui-com/certification) - Stateless Ownership of Digital Goods. Upon a product purchase user is given encrypted tamper-proof certificate that can be used to download/re-download the product.zip. 

## DRM
- [certification](https://github.com/fantasyui-com/certification) - Stateless Ownership of Digital Goods. Upon a product purchase user is given encrypted tamper-proof certificate that can be used to download/re-download the product.zip. 

## Dashboard
- [bootstrap-dashboard](https://github.com/fantasyui-com/bootstrap-dashboard) - Fully Functional Pure Bootstrap Dashboard, powered by EventEmitter, npm, Node and Electron. 

## Data Faker
- [bootstrap-faker](https://github.com/fantasyui-com/bootstrap-faker) - Bootstrap Faker generates massive amounts of useful randomized bootstrap related data such as card styles, warning labels, button types, etc. 

## Diff
- [diff-css](https://github.com/fantasyui-com/diff-css) - Command to compare two CSS files and list changes. 

## Electron
- [app-store](https://github.com/fantasyui-com/app-store) - Flexible general purpose app-store framework with customizable app feeds. 
- [application-installer](https://github.com/fantasyui-com/application-installer) - Quickly install npm modules and launch them in electron. 
- [application-launcher](https://github.com/fantasyui-com/application-launcher) - Easily install and launch electron applications from npm. Just set your electron application npm-name in configuration.json and ship the zips (Linux, Mac, Win). Launcher (which is electron+npm) will keep installing the latest version of your module each time it is executed. 
- [bootstrap-dashboard](https://github.com/fantasyui-com/bootstrap-dashboard) - Fully Functional Pure Bootstrap Dashboard, powered by EventEmitter, npm, Node and Electron. 
- [bootstrap-electron](https://github.com/fantasyui-com/bootstrap-electron) - Bootstrap based Electron Application. 
- [botnet](https://github.com/fantasyui-com/botnet) - Electron based communication program driven by robotic network entities inspired by email. 
- [business-software](https://github.com/fantasyui-com/business-software) - Open source app store for Business Managers. 
- [color-schemes](https://github.com/fantasyui-com/color-schemes) - Color Scheme Explorer (GUI Application). 
- [commode](https://github.com/fantasyui-com/commode) - Electron-Vue based, iTunes like UI, for file tagging, restructuring, processing, and general management. 
- [cornwall](https://github.com/fantasyui-com/cornwall) - Electron-based PixiJS Video Recording Environment. 
- [cylinder](https://github.com/fantasyui-com/cylinder) - Electron based digital audio workstation piano roll. 
- [epx](https://github.com/fantasyui-com/epx) - Electron Package Execute (npx but for electron apps). 
- [giblets](https://github.com/fantasyui-com/giblets) - Musical instrument based on giblet sound emitter. 
- [harlequin](https://github.com/fantasyui-com/harlequin) - Live Music Programming Environment. 
- [osmose](https://github.com/fantasyui-com/osmose) - Electron application for easy color balancing and filtering. 
- [pacom](https://github.com/fantasyui-com/pacom) - Minimalist WikiWiki with an english-like programming language. 
- [page-setup](https://github.com/fantasyui-com/page-setup) - Electron application for decorating and printing plain text files. 
- [physical-fitness](https://github.com/fantasyui-com/physical-fitness) - Open source app store for nerds and bodybuilders. 
- [readme-editor](https://github.com/fantasyui-com/readme-editor) - Super simple readme editor (app-store example package). 
- [shortwave](https://github.com/fantasyui-com/shortwave) - Electron based music beat producer, powered by Node Emitter, Howler, Vue and Bootstrap. 
- [writing-workshop](https://github.com/fantasyui-com/writing-workshop) - Open source app store for poets, writers and philosophers. 
- [yip](https://github.com/fantasyui-com/yip) - Electron backed music player based on the howler audio library. 

## Event Emitter
- [event-server](https://github.com/fantasyui-com/event-server) - Serve node EventEmitter events over networks. 

## File Management
- [changeline](https://github.com/fantasyui-com/changeline) - Search and replace in lines of files across directories. 
- [commode](https://github.com/fantasyui-com/commode) - Electron-Vue based, iTunes like UI, for file tagging, restructuring, processing, and general management. 
- [kebab-files](https://github.com/fantasyui-com/kebab-files) - Smartly normalize filenames in a directory by stripping down filenames to alphanumeric characters with hyphens. 
- [line-filter](https://github.com/fantasyui-com/line-filter) - Command that controls echoing of lines from source files based on whitelists and blacklists. 
- [uid-files](https://github.com/fantasyui-com/uid-files) - Recursively move files renaming them to a unique id in the process. 

## Files
- [ultrasonic](https://github.com/fantasyui-com/ultrasonic) - A collection of outstanding sound samples extracted from sonic-pi project and made compatible with npm modules for use with node. 

## Fitness
- [undefeated](https://github.com/fantasyui-com/undefeated) - Virtual Trainer System for Rapid Body Development. 

## Hardened
- [hardened-express](https://github.com/fantasyui-com/hardened-express) - Fast, opinionated, hardened web framework. 

## Index Of
- [index](https://github.com/fantasyui-com/index) - fantasyui-com repositories in more detail. 

## JSON
- [product-catalog](https://github.com/fantasyui-com/product-catalog) - Efficient lodash backed, package.json formatted digital downloads product-catalog with minimal requirements and maintenance. 

## Language
- [civilized](https://github.com/fantasyui-com/civilized) - Means of programming without learning a programming language. 
- [conversational-ui](https://github.com/fantasyui-com/conversational-ui) - XRegExp based natural language interface to functions. 
- [lucid-programming](https://github.com/fantasyui-com/lucid-programming) - Simple, fast and minimalist programming language for programming via SMS, MUD, IRC, etc. 

## Launcher
- [application-installer](https://github.com/fantasyui-com/application-installer) - Quickly install npm modules and launch them in electron. 

## Library
- [application-installer](https://github.com/fantasyui-com/application-installer) - Quickly install npm modules and launch them in electron. 
- [aquarium](https://github.com/fantasyui-com/aquarium) - A simple proportional value distribution library aimed at performing probability driven experiments in an enclosed virtual space by means of virtual objects and user defined uncertainty. 
- [beatnik](https://github.com/fantasyui-com/beatnik) - Flexible Musical Beat Stream Generator. 
- [bonsoir](https://github.com/fantasyui-com/bonsoir) - Node based server status program. 
- [career](https://github.com/fantasyui-com/career) - Career Description File Format for Resume/CV Applications. 
- [catalog-server](https://github.com/fantasyui-com/catalog-server) - Commercial product management server. 
- [certification](https://github.com/fantasyui-com/certification) - Stateless Ownership of Digital Goods. Upon a product purchase user is given encrypted tamper-proof certificate that can be used to download/re-download the product.zip. 
- [cyberpunk](https://github.com/fantasyui-com/cyberpunk) - A cyberpunk music generator. 
- [data-url-to-file](https://github.com/fantasyui-com/data-url-to-file) - Save arbitrary dataURL (data URI) to a file, with mime support and standard node callback. 
- [decrement](https://github.com/fantasyui-com/decrement) - Auto decrement value at preset rate. 
- [diff-css](https://github.com/fantasyui-com/diff-css) - Command to compare two CSS files and list changes. 
- [giblet](https://github.com/fantasyui-com/giblet) - General purpose event based synchronized sound emitter. 
- [list-css-classes](https://github.com/fantasyui-com/list-css-classes) - Node npm module used to parse a CSS string and list the CSS classes defined inside. 
- [ls-html](https://github.com/fantasyui-com/ls-html) - List information about HTML file. 
- [natural-sort-by-key](https://github.com/fantasyui-com/natural-sort-by-key) - Sort array of objects by natural, human, alphanumeric order. Natural sort order is an ordering of strings in alphabetical order, except that multi-digit numbers are ordered as a single character. 
- [oneof](https://github.com/fantasyui-com/oneof) - Return a random item from a list. 
- [select-declarations](https://github.com/fantasyui-com/select-declarations) - Select contents of rule declarations via CSS selectors. 
- [spacers](https://github.com/fantasyui-com/spacers) - Lightweight CSS spacing classes inspired by bootstrap generated with precss. 
- [squirm](https://github.com/fantasyui-com/squirm) - CSS Color Processing Library. 
- [uid-files](https://github.com/fantasyui-com/uid-files) - Recursively move files renaming them to a unique id in the process. 
- [web-fonts](https://github.com/fantasyui-com/web-fonts) - PostCSS web font injector (PostCSS plugin). 

## MUD
- [botnet](https://github.com/fantasyui-com/botnet) - Electron based communication program driven by robotic network entities inspired by email. 
- [cgui](https://github.com/fantasyui-com/cgui) - Conversational Graphic User Interface. 
- [chub](https://github.com/fantasyui-com/chub) - Crate subscription, management and shipping business in a box. 
- [food-delivery](https://github.com/fantasyui-com/food-delivery) - Massively Distributed Food Delivery Service Framework, 
- [limerick](https://github.com/fantasyui-com/limerick) - World Poetry Project. 
- [lucid-programming](https://github.com/fantasyui-com/lucid-programming) - Simple, fast and minimalist programming language for programming via SMS, MUD, IRC, etc. 
- [reproduction](https://github.com/fantasyui-com/reproduction) - Object oriented world of interactive things. 

## Multiprocess
- [multiprocess-store](https://github.com/fantasyui-com/multiprocess-store) - ES6-and-beyond multiprocess-safe file-system-based object-store. 

## Music
- [beatnik](https://github.com/fantasyui-com/beatnik) - Flexible Musical Beat Stream Generator. 
- [cyberpunk](https://github.com/fantasyui-com/cyberpunk) - A cyberpunk music generator. 
- [cylinder](https://github.com/fantasyui-com/cylinder) - Electron based digital audio workstation piano roll. 
- [giblet](https://github.com/fantasyui-com/giblet) - General purpose event based synchronized sound emitter. 
- [giblets](https://github.com/fantasyui-com/giblets) - Musical instrument based on giblet sound emitter. 
- [harlequin](https://github.com/fantasyui-com/harlequin) - Live Music Programming Environment. 
- [shortwave](https://github.com/fantasyui-com/shortwave) - Electron based music beat producer, powered by Node Emitter, Howler, Vue and Bootstrap. 
- [ultrasonic](https://github.com/fantasyui-com/ultrasonic) - A collection of outstanding sound samples extracted from sonic-pi project and made compatible with npm modules for use with node. 
- [yip](https://github.com/fantasyui-com/yip) - Electron backed music player based on the howler audio library. 

## Network
- [event-server](https://github.com/fantasyui-com/event-server) - Serve node EventEmitter events over networks. 

## Node
- [node.sh](https://github.com/fantasyui-com/node.sh) - Node JS Shell. 

## Object Database
- [multiprocess-store](https://github.com/fantasyui-com/multiprocess-store) - ES6-and-beyond multiprocess-safe file-system-based object-store. 
- [product-catalog](https://github.com/fantasyui-com/product-catalog) - Efficient lodash backed, package.json formatted digital downloads product-catalog with minimal requirements and maintenance. 
- [und](https://github.com/fantasyui-com/und) - Multiprocess safe file save. Don't just save files, that will cause errors. Rather, create a directory named after the file, and store UUID named writes within it. 
- [user-manager](https://github.com/fantasyui-com/user-manager) - Simple user manager for the people. 

## Ping
- [bonsoir](https://github.com/fantasyui-com/bonsoir) - Node based server status program. 

## Poem
- [kosmos](https://github.com/fantasyui-com/kosmos) - A poem about learning and growing up. 
- [limerick](https://github.com/fantasyui-com/limerick) - World Poetry Project. 
- [loneliness](https://github.com/fantasyui-com/loneliness) - a poem about the corrosiveness of loneliness. 
- [poems](https://github.com/fantasyui-com/poems) - Poetry Booklet Build System. 

## Printing
- [page-setup](https://github.com/fantasyui-com/page-setup) - Electron application for decorating and printing plain text files. 

## Proxy
- [message-server](https://github.com/fantasyui-com/message-server) - Beautiful, minimalist stateful server for converting internet into a list of messages with title, body, comments, and read/unread states. 

## Resume
- [career](https://github.com/fantasyui-com/career) - Career Description File Format for Resume/CV Applications. 

## Robots
- [botnet](https://github.com/fantasyui-com/botnet) - Electron based communication program driven by robotic network entities inspired by email. 

## Server
- [bootstrap-cards](https://github.com/fantasyui-com/bootstrap-cards) - Infinite Number of Bootstrap Cards. 
- [bootstrap-containers](https://github.com/fantasyui-com/bootstrap-containers) - Bootstrap Container Components. 
- [hardened-express](https://github.com/fantasyui-com/hardened-express) - Fast, opinionated, hardened web framework. 
- [message-server](https://github.com/fantasyui-com/message-server) - Beautiful, minimalist stateful server for converting internet into a list of messages with title, body, comments, and read/unread states. 
- [poems](https://github.com/fantasyui-com/poems) - Poetry Booklet Build System. 
- [vicious](https://github.com/fantasyui-com/vicious) - Business in a box server. Inspired by google result pages driven via payments by stripe. 

## Shell
- [node.sh](https://github.com/fantasyui-com/node.sh) - Node JS Shell. 

## Show and Tell
- [photograph](https://github.com/fantasyui-com/photograph) - Wrap web application UI in a pretty frame, prior to screen capture and posting to show and tell sites. 

## Smart CSS
- [cgui](https://github.com/fantasyui-com/cgui) - Conversational Graphic User Interface. 

## Sort
- [natural-sort-by-key](https://github.com/fantasyui-com/natural-sort-by-key) - Sort array of objects by natural, human, alphanumeric order. Natural sort order is an ordering of strings in alphabetical order, except that multi-digit numbers are ordered as a single character. 

## StackScript
- [online-marketplace](https://github.com/fantasyui-com/online-marketplace) - Simple online marketplace for selling files. 

## Standard Library
- [oneof](https://github.com/fantasyui-com/oneof) - Return a random item from a list. 

## UI
- [card-generator](https://github.com/fantasyui-com/card-generator) - Bootstrap Card Generator. 
- [cgui](https://github.com/fantasyui-com/cgui) - Conversational Graphic User Interface. 
- [container-generator](https://github.com/fantasyui-com/container-generator) - Bootstrap Container Generator. 
- [message-server](https://github.com/fantasyui-com/message-server) - Beautiful, minimalist stateful server for converting internet into a list of messages with title, body, comments, and read/unread states. 
- [skins](https://github.com/fantasyui-com/skins) - Library of Industry Standard CSS Designs. 
- [spqr](https://github.com/fantasyui-com/spqr) - Webpage Design Generator. 

## URL
- [data-url-to-file](https://github.com/fantasyui-com/data-url-to-file) - Save arbitrary dataURL (data URI) to a file, with mime support and standard node callback. 

## VFS
- [node.sh](https://github.com/fantasyui-com/node.sh) - Node JS Shell. 

## Video
- [cornwall](https://github.com/fantasyui-com/cornwall) - Electron-based PixiJS Video Recording Environment. 

## Web Application
- [bootstrap-cards](https://github.com/fantasyui-com/bootstrap-cards) - Infinite Number of Bootstrap Cards. 
- [bootstrap-containers](https://github.com/fantasyui-com/bootstrap-containers) - Bootstrap Container Components. 
- [message-server](https://github.com/fantasyui-com/message-server) - Beautiful, minimalist stateful server for converting internet into a list of messages with title, body, comments, and read/unread states. 
- [online-marketplace](https://github.com/fantasyui-com/online-marketplace) - Simple online marketplace for selling files. 
- [poems](https://github.com/fantasyui-com/poems) - Poetry Booklet Build System. 
- [spqr](https://github.com/fantasyui-com/spqr) - Webpage Design Generator. 
- [undefeated](https://github.com/fantasyui-com/undefeated) - Virtual Trainer System for Rapid Body Development. 
- [vicious](https://github.com/fantasyui-com/vicious) - Business in a box server. Inspired by google result pages driven via payments by stripe. 

## Web Design
- [color-schemes](https://github.com/fantasyui-com/color-schemes) - Color Scheme Explorer (GUI Application). 
- [larceny](https://github.com/fantasyui-com/larceny) - Transport select remote CSS declarations into the rule of a local css file (Select and inject remote CSS code into your local CSS file). 
- [list-html-classes](https://github.com/fantasyui-com/list-html-classes) - Node npm module used to parse HTML string and list the CSS classes. 
- [ls-css](https://github.com/fantasyui-com/ls-css) - List information about CSS file. 
- [osmose](https://github.com/fantasyui-com/osmose) - Electron application for easy color balancing and filtering. 
- [revolutionize](https://github.com/fantasyui-com/revolutionize) - PostCSS based web application skinning strategy. 
- [spqr](https://github.com/fantasyui-com/spqr) - Webpage Design Generator. 
- [theft](https://github.com/fantasyui-com/theft) - Means of transporting CSS rule declarations from one file to another. 

## WikiWiki
- [pacom](https://github.com/fantasyui-com/pacom) - Minimalist WikiWiki with an english-like programming language. 

## Index
- [app-catalog](https://github.com/fantasyui-com/app-catalog) - Application catalog for npm's app-store. 
- [app-store](https://github.com/fantasyui-com/app-store) - Flexible general purpose app-store framework with customizable app feeds. 
- [application-installer](https://github.com/fantasyui-com/application-installer) - Quickly install npm modules and launch them in electron. 
- [application-launcher](https://github.com/fantasyui-com/application-launcher) - Easily install and launch electron applications from npm. Just set your electron application npm-name in configuration.json and ship the zips (Linux, Mac, Win). Launcher (which is electron+npm) will keep installing the latest version of your module each time it is executed. 
- [aquarium](https://github.com/fantasyui-com/aquarium) - A simple proportional value distribution library aimed at performing probability driven experiments in an enclosed virtual space by means of virtual objects and user defined uncertainty. 
- [awesome-gui](https://github.com/fantasyui-com/awesome-gui) - Awesome list of GUI concepts. 
- [awesome-http](https://github.com/fantasyui-com/awesome-http) - Awesome HTTP. 
- [awesome-ideas](https://github.com/fantasyui-com/awesome-ideas) - Awesome programming ideas and inspiration from all around the world of languages and patterns. 
- [awesome-publishing](https://github.com/fantasyui-com/awesome-publishing) - Awesome List of Modern Publishing Resources. 
- [awesome](https://github.com/fantasyui-com/awesome) - A short list of awesome people. 
- [beatnik](https://github.com/fantasyui-com/beatnik) - Flexible Musical Beat Stream Generator. 
- [bonsoir](https://github.com/fantasyui-com/bonsoir) - Node based server status program. 
- [bootstrap-cards](https://github.com/fantasyui-com/bootstrap-cards) - Infinite Number of Bootstrap Cards. 
- [bootstrap-containers](https://github.com/fantasyui-com/bootstrap-containers) - Bootstrap Container Components. 
- [bootstrap-dashboard](https://github.com/fantasyui-com/bootstrap-dashboard) - Fully Functional Pure Bootstrap Dashboard, powered by EventEmitter, npm, Node and Electron. 
- [bootstrap-electron](https://github.com/fantasyui-com/bootstrap-electron) - Bootstrap based Electron Application. 
- [bootstrap-faker](https://github.com/fantasyui-com/bootstrap-faker) - Bootstrap Faker generates massive amounts of useful randomized bootstrap related data such as card styles, warning labels, button types, etc. 
- [bootstrap-monitor](https://github.com/fantasyui-com/bootstrap-monitor) - Bootstrap 4 monitoring system designed to help tracking class changes. 
- [bootstrap-styleguide](https://github.com/fantasyui-com/bootstrap-styleguide) - Bootstrap Styleguide generator. Create bootstrap style guides by scraping documentation. 
- [botnet](https://github.com/fantasyui-com/botnet) - Electron based communication program driven by robotic network entities inspired by email. 
- [business-software](https://github.com/fantasyui-com/business-software) - Open source app store for Business Managers. 
- [card-generator](https://github.com/fantasyui-com/card-generator) - Bootstrap Card Generator. 
- [career](https://github.com/fantasyui-com/career) - Career Description File Format for Resume/CV Applications. 
- [catalog-server](https://github.com/fantasyui-com/catalog-server) - Commercial product management server. 
- [certification](https://github.com/fantasyui-com/certification) - Stateless Ownership of Digital Goods. Upon a product purchase user is given encrypted tamper-proof certificate that can be used to download/re-download the product.zip. 
- [cgui](https://github.com/fantasyui-com/cgui) - Conversational Graphic User Interface. 
- [changeline](https://github.com/fantasyui-com/changeline) - Search and replace in lines of files across directories. 
- [chub](https://github.com/fantasyui-com/chub) - Crate subscription, management and shipping business in a box. 
- [civilized](https://github.com/fantasyui-com/civilized) - Means of programming without learning a programming language. 
- [color-schemes](https://github.com/fantasyui-com/color-schemes) - Color Scheme Explorer (GUI Application). 
- [commode](https://github.com/fantasyui-com/commode) - Electron-Vue based, iTunes like UI, for file tagging, restructuring, processing, and general management. 
- [container-generator](https://github.com/fantasyui-com/container-generator) - Bootstrap Container Generator. 
- [conversational-ui](https://github.com/fantasyui-com/conversational-ui) - XRegExp based natural language interface to functions. 
- [cornwall](https://github.com/fantasyui-com/cornwall) - Electron-based PixiJS Video Recording Environment. 
- [csshue](https://github.com/fantasyui-com/csshue) - CLI utility to uniformly rotate hue of colors in a css file. 
- [cyberpunk](https://github.com/fantasyui-com/cyberpunk) - A cyberpunk music generator. 
- [cylinder](https://github.com/fantasyui-com/cylinder) - Electron based digital audio workstation piano roll. 
- [data-url-to-file](https://github.com/fantasyui-com/data-url-to-file) - Save arbitrary dataURL (data URI) to a file, with mime support and standard node callback. 
- [decrement](https://github.com/fantasyui-com/decrement) - Auto decrement value at preset rate. 
- [diff-css](https://github.com/fantasyui-com/diff-css) - Command to compare two CSS files and list changes. 
- [epx](https://github.com/fantasyui-com/epx) - Electron Package Execute (npx but for electron apps). 
- [event-server](https://github.com/fantasyui-com/event-server) - Serve node EventEmitter events over networks. 
- [food-delivery](https://github.com/fantasyui-com/food-delivery) - Massively Distributed Food Delivery Service Framework, 
- [giblet](https://github.com/fantasyui-com/giblet) - General purpose event based synchronized sound emitter. 
- [giblets](https://github.com/fantasyui-com/giblets) - Musical instrument based on giblet sound emitter. 
- [hardened-express](https://github.com/fantasyui-com/hardened-express) - Fast, opinionated, hardened web framework. 
- [harlequin](https://github.com/fantasyui-com/harlequin) - Live Music Programming Environment. 
- [id3-files](https://github.com/fantasyui-com/id3-files) - Employ ID v3 information in renaming of files. [ID3, MP3, Music]
- [illustrator](https://github.com/fantasyui-com/illustrator) - Command line tool to extract drawing reference from a photo and create a pdf formatted drawing reference booklet. A tool for artists of all ages. 
- [index](https://github.com/fantasyui-com/index) - fantasyui-com repositories in more detail. 
- [kebab-files](https://github.com/fantasyui-com/kebab-files) - Smartly normalize filenames in a directory by stripping down filenames to alphanumeric characters with hyphens. 
- [kosmos](https://github.com/fantasyui-com/kosmos) - A poem about learning and growing up. 
- [larceny](https://github.com/fantasyui-com/larceny) - Transport select remote CSS declarations into the rule of a local css file (Select and inject remote CSS code into your local CSS file). 
- [limerick](https://github.com/fantasyui-com/limerick) - World Poetry Project. 
- [line-filter](https://github.com/fantasyui-com/line-filter) - Command that controls echoing of lines from source files based on whitelists and blacklists. 
- [list-css-classes](https://github.com/fantasyui-com/list-css-classes) - Node npm module used to parse a CSS string and list the CSS classes defined inside. 
- [list-html-classes](https://github.com/fantasyui-com/list-html-classes) - Node npm module used to parse HTML string and list the CSS classes. 
- [loneliness](https://github.com/fantasyui-com/loneliness) - a poem about the corrosiveness of loneliness. 
- [ls-css](https://github.com/fantasyui-com/ls-css) - List information about CSS file. 
- [ls-html](https://github.com/fantasyui-com/ls-html) - List information about HTML file. 
- [lucid-programming](https://github.com/fantasyui-com/lucid-programming) - Simple, fast and minimalist programming language for programming via SMS, MUD, IRC, etc. 
- [malformed](https://github.com/fantasyui-com/malformed) - List malformed music metadata. [CLI, Analysis, MP3]
- [message-server](https://github.com/fantasyui-com/message-server) - Beautiful, minimalist stateful server for converting internet into a list of messages with title, body, comments, and read/unread states. 
- [mockup](https://github.com/fantasyui-com/mockup) - Perspective app screens and isometric mock-up tool. Automated high quality, high resolution, marketing art designer for mockups. Written in minimalist Bash using ImageMagick and Love! 
- [multiprocess-store](https://github.com/fantasyui-com/multiprocess-store) - ES6-and-beyond multiprocess-safe file-system-based object-store. 
- [natural-sort-by-key](https://github.com/fantasyui-com/natural-sort-by-key) - Sort array of objects by natural, human, alphanumeric order. Natural sort order is an ordering of strings in alphabetical order, except that multi-digit numbers are ordered as a single character. 
- [node.sh](https://github.com/fantasyui-com/node.sh) - Node JS Shell. 
- [oneof](https://github.com/fantasyui-com/oneof) - Return a random item from a list. 
- [online-marketplace](https://github.com/fantasyui-com/online-marketplace) - Simple online marketplace for selling files. 
- [osmose](https://github.com/fantasyui-com/osmose) - Electron application for easy color balancing and filtering. 
- [pacom](https://github.com/fantasyui-com/pacom) - Minimalist WikiWiki with an english-like programming language. 
- [page-setup](https://github.com/fantasyui-com/page-setup) - Electron application for decorating and printing plain text files. 
- [photograph](https://github.com/fantasyui-com/photograph) - Wrap web application UI in a pretty frame, prior to screen capture and posting to show and tell sites. 
- [physical-fitness](https://github.com/fantasyui-com/physical-fitness) - Open source app store for nerds and bodybuilders. 
- [poems](https://github.com/fantasyui-com/poems) - Poetry Booklet Build System. 
- [product-catalog](https://github.com/fantasyui-com/product-catalog) - Efficient lodash backed, package.json formatted digital downloads product-catalog with minimal requirements and maintenance. 
- [readme-editor](https://github.com/fantasyui-com/readme-editor) - Super simple readme editor (app-store example package). 
- [recombinator](https://github.com/fantasyui-com/recombinator) - CSS recombination system. 
- [reproduction](https://github.com/fantasyui-com/reproduction) - Object oriented world of interactive things. 
- [revolutionize](https://github.com/fantasyui-com/revolutionize) - PostCSS based web application skinning strategy. 
- [select-declarations](https://github.com/fantasyui-com/select-declarations) - Select contents of rule declarations via CSS selectors. 
- [shortwave](https://github.com/fantasyui-com/shortwave) - Electron based music beat producer, powered by Node Emitter, Howler, Vue and Bootstrap. 
- [skins](https://github.com/fantasyui-com/skins) - Library of Industry Standard CSS Designs. 
- [spacers](https://github.com/fantasyui-com/spacers) - Lightweight CSS spacing classes inspired by bootstrap generated with precss. 
- [spqr](https://github.com/fantasyui-com/spqr) - Webpage Design Generator. 
- [squirm](https://github.com/fantasyui-com/squirm) - CSS Color Processing Library. 
- [theft](https://github.com/fantasyui-com/theft) - Means of transporting CSS rule declarations from one file to another. 
- [uid-files](https://github.com/fantasyui-com/uid-files) - Recursively move files renaming them to a unique id in the process. 
- [ultrasonic](https://github.com/fantasyui-com/ultrasonic) - A collection of outstanding sound samples extracted from sonic-pi project and made compatible with npm modules for use with node. 
- [und](https://github.com/fantasyui-com/und) - Multiprocess safe file save. Don't just save files, that will cause errors. Rather, create a directory named after the file, and store UUID named writes within it. 
- [undefeated](https://github.com/fantasyui-com/undefeated) - Virtual Trainer System for Rapid Body Development. 
- [user-manager](https://github.com/fantasyui-com/user-manager) - Simple user manager for the people. 
- [vicious](https://github.com/fantasyui-com/vicious) - Business in a box server. Inspired by google result pages driven via payments by stripe. 
- [web-fonts](https://github.com/fantasyui-com/web-fonts) - PostCSS web font injector (PostCSS plugin). 
- [writing-workshop](https://github.com/fantasyui-com/writing-workshop) - Open source app store for poets, writers and philosophers. 
- [yip](https://github.com/fantasyui-com/yip) - Electron backed music player based on the howler audio library. 

